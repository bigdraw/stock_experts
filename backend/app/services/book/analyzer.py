    ...
    async def _analyze_chunk(self, text: str, index: int, total: int) -> str:
        ...
        response = await self.llm.chat(
            [
                LLMMessage(
                    role="system",
                    content="""你是一位投资书籍分析专家。请从以下文本中提取：
1. 核心投资理念和哲学
2. 具体的投资策略和规则
3. 风险管理和仓位控制方法
4. 选股标准和评估框架
    ...
    async def _synthesize(self, analyses: list[str], book_title: str) -> dict:
        ...
        response = await self.llm.chat(
            [
                LLMMessage(
                    role="system",
                    content=f"""你是投资 Agent 定义专家。基于以下对《{book_title}》的分析结果，
生成一个完整的投资 Agent 定义。
...
只输出 JSON，不要解释。""",
        ...
        try:
            ...
            raise ValueError("Failed to parse agent definition from LLM response") from None
