...
class DataAcquisitionEngine:
    ...
    async def _persist_log_outcome(
        self,
        log_id: int | None,
        status: str,
    ...
    async def full_basic_collection(
        self, batch_size: int = 100, task_id: str = None, code_prefixes: list[str] = None
    ) -> str:
        ...
        try:
            ...
            task_manager.update_progress(
                task_id, message=f"Fetching indicators for {code_prefixes} stocks..."
            )
            ...
            for i in range(0, total_indicators, batch_size):
                ...
                if not task_manager.wait_if_paused(task_id):
                    ...
                    logger.info(
                        f"Full collection stopped by user: {processed}/{total_indicators} stocks"
                    )
                    return task_id
                ...
                for ind in batch:
                    ...
                    if not validation_result.is_valid:
                        logger.warning(
                            f"Skipping invalid data for {ind.code}: {validation_result.errors}"
                        )
    ...
    async def incremental_update(self, task_id: str = None) -> str:
        ...
        try:
            ...
            result = await self.db.execute(select(Stock).where(Stock.is_active))
                    ...
                    for q in quotes:
                        existing = await self.db.execute(
                            select(DailyQuoteModel).where(
                                ...
                                DailyQuoteModel.date
                                == datetime.strptime(q.date, "%Y-%m-%d").date(),
                            )
                        )
        ...
        try:
            ...
            for code in stock_codes:
                ...
                try:
                    ...
                    for r in reports:
                        report_date = (
                            datetime.strptime(r.report_date, "%Y-%m-%d").date()
                            if r.report_date
    ...
    async def full_financial_update(self, task_id: str = None) -> str:
        ...
        try:
            ...
            task_manager.update_progress(
                task_id, message=f"Updating financial data for {total} stocks..."
            )
