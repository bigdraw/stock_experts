    ...
    def validate(cls, ind: StockBasicIndicators) -> ValidationResult:
        ...
        if ind.circulating_market_cap is not None:
            if ind.circulating_market_cap < 0:
                ...
                warnings.append(
                    f"Circulating market cap out of range: {ind.circulating_market_cap}"
                )
        ...
        if ind.changepercent is not None:
            if (
                ind.changepercent < cls.RANGES["changepercent"][0]
                or ind.changepercent > cls.RANGES["changepercent"][1]
            ):
